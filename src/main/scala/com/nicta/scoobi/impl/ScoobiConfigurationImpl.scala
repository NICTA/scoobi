...
case class ScoobiConfigurationImpl(private val hadoopConfiguration: Configuration = new Configuration,
   ...
   */
  if (hadoopConfiguration.get(SCOOBI_MODE) == null) {
  ...
  def updateCounters(hadoopCounters: HadoopCounters) = {
    Option(hadoopCounters.getGroupNames).toList.flatten.foreach { groupName: String =>
      Option(hadoopCounters.getGroup(groupName)).toList.flatMap(_.iterator.toList).foreach { c: Counter =>
  ...
  def temporaryOutputDirectory            = fileSystem.makeQualified(new Path(temporaryDir))
