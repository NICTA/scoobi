...
case class InMemoryMode() extends ExecutionMode {
  ...
  lazy val compute: ScoobiConfiguration => CompNode => Seq[_] =
    paramAttr { sc: ScoobiConfiguration => node: CompNode =>
      ...
      if (isExpectedResult(node))
        node match {
          case Materialise1(_) | Op1(_,_) | GroupByKey1(_) | ParallelDo1(_) | Combine1(_) | Return1(_) => saveSinks(result, node)
          case _                                                                                     => ()
        }
      result
    }
  ...
  private def isExpectedResult = (node: CompNode) =>
    !parent(node).isDefined || parent(node).exists(isRoot) ||
    ...
    node.hasCheckpoint && !(parent(node).exists(isMaterialise) && parent(node).exists(_.sinks.isEmpty))
