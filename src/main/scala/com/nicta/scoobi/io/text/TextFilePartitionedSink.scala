...
/**
 * This text format creates a new record writer for each different path that's generated by the partition function
...
class PartitionedTextOutputFormat[K, V] extends FileOutputFormat[K, V] { outer =>
  ...
  def getRecordWriter(context: TaskAttemptContext): RecordWriter[K, V] = {
    ...
    new RecordWriter[K, V] {
      ...
      def write(key: K, value: V) {
        val finalPath = generatePathForKeyValue(sinkId, key, value, workDir, partitionFunction)(context.getConfiguration)
  ...
  protected def generatePathForKeyValue(sinkId: String, key: K, value: V, workDir: Path, partitionFunction: Option[K => String])(configuration: Configuration): Path = {
    new Path(workDir, new Path(sinkId, partitionFunction.map(_(key)).getOrElse(key.toString)))
  }
