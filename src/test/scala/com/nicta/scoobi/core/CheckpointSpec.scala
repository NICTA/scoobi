...
class CheckpointSpec extends UnitSpecification with Mockito {
  "A checkpoint exists on a Sink if the sink is a checkpoint and if there are files with previous results in the output directory" >> {
    ...
    implicit val configuration = mock[ScoobiConfiguration]
    configuration.fileSystem returns mockFs

    val checkpoint = avroSink[Int](TestFiles.createTempFile("test")(new ScoobiConfigurationImpl).getPath, checkpoint = true)
    ...
    mockFs.listStatus(any[Path]) returns Array(status)
    status.getPath returns new Path("_SUCCESS")
