...
trait CompNodeData extends Data with ScalaCheckMatchers with CompNodeFactory { outer =>
  ...
  def genList1(depth: Int = 1): Gen[DList[String]] =
    if (depth <= 1) Gen.const(DList("source"))
...
trait CompNodeFactory extends Scope {
  ...
  def cb(in: CompNode)                 = Combine(in, Combine.reducer((s1: Any, s2: Any) => s1.toString + s2.toString), wireFormat[String], wireFormat[String], wireFormat[String])
