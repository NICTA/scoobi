...
See the [install instructions](${SCOOBI_GUIDE_OFFICIAL}/Quick%20Start.html#Installing+Scoobi) in the QuickStart section of the [User Guide](${SCOOBI_GUIDE_OFFICIAL}/User%20Guide.html).
 ...
 * Ability to parameterize with rich [data types](${SCOOBI_GUIDE_OFFICIAL}/Data%20Types.html) - unlike Hadoop MapReduce, which requires that you go off implementing a myriad of classes that implement the `Writable` interface, Scoobi allows `DList` objects to be parameterized by normal Scala types including value types (e.g. `Int`, `String`, `Double`), tuple types (with arbitrary nesting) as well as **case classes**

 * Support for multiple types of I/O - currently built-in support for [text](${SCOOBI_GUIDE_OFFICIAL}/Input%20and%20Output.html#Text+files), [Sequence](${SCOOBI_GUIDE_OFFICIAL}/Input%20and%20Output.html#Sequence+files) and [Avro](${SCOOBI_GUIDE_OFFICIAL}/Input%20and%20Output.html#Avro+files) files with the ability to implement support for [custom sources/sinks](${SCOOBI_GUIDE_OFFICIAL}/Input%20and%20Output.html#Custom+sources+and+sinks)
...
To get started, read the [getting started steps](${SCOOBI_GUIDE_OFFICIAL}/Quick%20Start.html) and the section on [distributed lists](${SCOOBI_GUIDE_OFFICIAL}/Distributed%20Lists.html). The remaining sections in the [User Guide](${SCOOBI_GUIDE_OFFICIAL}/User%20Guide.html) provide further detail on various aspects of Scoobi's functionality.
 ...
 * [API Documentation](http://nicta.github.com/scoobi/api/${SCOOBI_BRANCH}/index.html)
 * [Examples](https://github.com/NICTA/scoobi/tree/${SCOOBI_BRANCH}/examples)
 * User Guide for the [SNAPSHOT](${SCOOBI_GUIDE_SNAPSHOT}/User%20Guide.html) version ([latest api](http://nicta.github.com/scoobi/api/master/index.html))
